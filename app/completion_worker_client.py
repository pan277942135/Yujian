...
def check_completion_worker() -> dict[str, Any]:
    ...
    if not base_url:
        return {"endpoint_configured": False, "status": "WORKER_UNAVAILABLE", "reason": "worker_endpoint_not_configured"}
    req = urllib.request.Request(f"{base_url}/health", method="GET", headers=_request_headers())
    timeout = float(os.getenv("FISH_COMPLETION_WORKER_TIMEOUT_SECONDS", "900"))
    try:
        with urllib.request.urlopen(req, timeout=min(timeout, 30.0)) as response:
            body = response.read()
            status_code = response.status
    except urllib.error.HTTPError as exc:
        raise CompletionWorkerError("COMPLETION_WORKER_HEALTH_ERROR", exc.read().decode("utf-8", "replace")[:2000] or str(exc), status_code=exc.code) from exc
    except (urllib.error.URLError, TimeoutError) as exc:
        raise CompletionWorkerError("COMPLETION_WORKER_HEALTH_UNREACHABLE", str(exc)) from exc
    try:
        payload = json.loads(body.decode("utf-8")) if body else {}
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise CompletionWorkerError("COMPLETION_WORKER_HEALTH_INVALID_JSON", "Worker health returned non-JSON", status_code=status_code) from exc
    return {"endpoint_configured": True, "status": "READY" if 200 <= status_code < 300 else "WORKER_UNAVAILABLE", "health_status": status_code, "health": payload if isinstance(payload, dict) else {}}
...
def invoke_completion_worker(*, image_uri: str, mask_uri: str, prompt: str) -> dict[str, Any]:
    ...
    if not isinstance(result, dict):
        raise CompletionWorkerError("COMPLETION_WORKER_INVALID_RESPONSE", "Worker response is not an object")
    missing = [key for key in ("result_uri", "model_version", "inference_time_ms") if not result.get(key)]
    if missing:
        raise CompletionWorkerError("COMPLETION_WORKER_INVALID_RESPONSE", f"Worker response is missing: {', '.join(missing)}", status_code=status_code)
    return result
