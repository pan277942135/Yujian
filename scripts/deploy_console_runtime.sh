...
QWEN_GPU_PROJECT_ID="${QWEN_GPU_PROJECT_ID:-gen-lang-client-0224022273}"
QWEN_GPU_ZONE="${QWEN_GPU_ZONE:-us-central1-a}"
QWEN_GPU_INSTANCE="${QWEN_GPU_INSTANCE:-fish-completion-worker}"
QWEN_GPU_NAME="${QWEN_GPU_NAME:-NVIDIA L4}"
...
')"
  FISH_PORTRAIT_URL="$(printf '%s' "$PREVIOUS_SERVICE_JSON" | python -c '
import json,sys
d=json.load(sys.stdin)
containers=((d.get("spec") or {}).get("template") or {}).get("spec",{}).get("containers") or []
env=(containers[0].get("env") if containers else []) or []
print(next((x.get("value","") for x in env if x.get("name")=="FISH_PORTRAIT_WORKER_URL"), ""))
')"
  FISH_PORTRAIT_TOKEN="$(printf '%s' "$PREVIOUS_SERVICE_JSON" | python -c '
import json,sys
d=json.load(sys.stdin)
containers=((d.get("spec") or {}).get("template") or {}).get("spec",{}).get("containers") or []
env=(containers[0].get("env") if containers else []) or []
...
QWEN_WORKER_BASE_URL="${QWEN_WORKER_BASE_URL:-$FISH_QWEN_URL}"
DEPLOY_ENV_VARS="$DEPLOY_ENV_VARS,QWEN_GPU_PROJECT_ID=$QWEN_GPU_PROJECT_ID,QWEN_GPU_ZONE=$QWEN_GPU_ZONE,QWEN_GPU_INSTANCE=$QWEN_GPU_INSTANCE,QWEN_GPU_NAME=$QWEN_GPU_NAME,QWEN_GPU_WORKER=$QWEN_GPU_WORKER,QWEN_GPU_MODEL=$QWEN_GPU_MODEL"
if [[ -n "$QWEN_WORKER_BASE_URL" ]]; then
  DEPLOY_ENV_VARS="$DEPLOY_ENV_VARS,QWEN_WORKER_BASE_URL=$QWEN_WORKER_BASE_URL"
fi
