...
class DatasetCropReview(Base):
    ...
    quality_status = Column(String(32), index=True)
    all_detections_json = Column(Text)
    detector_error = Column(Text)
    accepted_bbox_json = Column(Text)
    bbox_source = Column(String(64))
    crop_uri = Column(Text)
    crop_status = Column(String(32), index=True)
    crop_error = Column(Text)
