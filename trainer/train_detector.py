...
def load_trusted_yolox_checkpoint(path: Path):
    """Load a checkpoint produced inside this fixed YOLOX training container.

    PyTorch 2.6+ defaults torch.load(weights_only=True), while YOLOX 0.3 stores
    optimizer/metadata values including NumPy scalars in its checkpoint. These files
    are generated by this job from fixed source and never come from user uploads, so
    the full trusted checkpoint format is intentionally loaded here.
    """
...
def run_training(dataset_root: Path, pretrain: Path, output_root: Path) -> tuple[Path, str, int]:
    ...
    latest_doc = load_trusted_yolox_checkpoint(latest_checkpoint)
...
def evaluate_checkpoint(checkpoint: Path) -> dict:
    ...
    exp = Exp()
    model = exp.get_model()
    state = load_trusted_yolox_checkpoint(checkpoint)
...
def main() -> None:
    ...
    checkpoint_doc = load_trusted_yolox_checkpoint(checkpoint)
    metadata = {
        "model_version": model_version,
